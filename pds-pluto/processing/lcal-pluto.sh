#!/bin/bash
# General pipeline for LORRI images

p=`pwd`
echo "LORRI Calib $p" >> ../cal.log
...
do
   ...
   echo "----- " $root  " ----- $cnt of $imgNum"
	...
	#############################
	echo "-------------------------------------- Convert"
	lorri2isis FROM=$root.fit TO=$framePath.cub
...
#      	EXTRA=$vSupport/support/HAMO_V1/dawn_vesta_v04-iau.tpc
	...
	isis2std FROM=$framePath.cub TO=frame/$root.tif format=tiff MAXPERCENT=99.9 
